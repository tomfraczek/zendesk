    ...
    for (let i = 0; i <= submenuSmall.length; i++){
        if(submenuSmall[i]){
            ...
            submenuSmall[i].addEventListener('click', (e) => {
                ...
                hiFiDropdown.style.height = (preList + e.target.parentElement.nextElementSibling.scrollHeight + e.target.parentElement.nextElementSibling.scrollHeight + hiFiDefault - (hiFiDefault + document.querySelector("#preList").scrollHeight)) + 'px';
